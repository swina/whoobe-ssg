<template>
    <div v-if="status.dialog" class="z-modal fixed inset-0 h-screen w-screen bg-black bg-opacity-30 items-center justify-center flex flex-col" style="z-index:999999999;" id="contextDialog">
        <div class="flex flex-col m-auto bg-white z-modal relative rounded-2xl" :class="status.dialogCss"> 
            <div class="bg-purple-700 text-white text-2xl flex items-center p-3 rounded-t-2xl">{{ status.dialogTitle || 'Dialog' }}</div>
            <span class="absolute right-0 top-0 m-3 text-2xl text-white cursor-pointer" @click="status.dialog=null"><icon icon="mdi:close"/></span>
            <BlockToProject v-if="status.dialog==='BlockToProject'"/>
            <CreateFolder v-if="status.dialog === 'CreateFolder'"/>
            <CreateFile v-if="status.dialog === 'CreateFile'"/>
            <DeleteFile v-if="status.dialog === 'Delete'"/>
            <UploadFile v-if="status.dialog === 'UploadFile'"/>
            <RenameFile v-if="status.dialog === 'RenameFile'"/>
            <DocumentSettings v-if="status.dialog === 'DocumentSettings'" :mode="status.mode"/>
            <BuilderNewPage v-if="store.status.dialog === 'BuilderNewPage'"/>
            <BlockTipTap v-if="status.dialog==='wysiwyg'"/>
            <BlockGrid v-if="status.dialog==='Grid'"/>
            <GraphQLClient v-if="status.dialog === 'GraphQL'"/>
            <ConfirmAction v-if="status.dialog === 'ConfirmAction'"/>
            <Images v-if="status.dialog === 'images'" context="images"/>
        </div>
    </div>
</template>

<script setup lang="ts">
import { ref } from 'vue'
//import { status } from '/@/composables/useNavigation';
import { openContextDialog , closeCtx } from '/@/composables/contextMenu';
import { store } from '/@/composables/useStore'
let status = store.status
</script>